  ...
  class Items
  {
      ...
      public function renderByBrand($make_id)
      {
		  ...
		  SELECT 
			...
			l.idx, l.nice_title, l.title, l.slug, l.price, l.price_sale, l.year, l.sold, l.thumb, l.created, l.featured
		  FROM
			`" . self::liTable . "` AS li 
      ...
      public function quickMessage()
      {
          ...
          if (empty(Message::$msgs)) {
              $numSent = 0;
              $mailer = Mailer::sendMail();
              $row = self::$db->select(self::mTable, array('email', 'CONCAT(fname," ",lname) as name'), array('id' => Filter::$id))->result();
